			  ...
			  <main class="container addForm">
			     <form method="post" name="vote">
			            <article class="form-group">
			               ...
			               <input name="title" value="<s:property value="questionnaireProject.title"/>"  class="form-control" id="title" placeholder="填写活动名称" check-type="required" required-message="活动名称不能为空"/>
			                <input name="id" value="<s:property value="questionnaireProject.id"/>"  class="form-control" id="id" type="hidden"/>
			            </article>
			              <article class="form-group">
			               <label for="organization">组织方</label>
			               <input name="organization" value="<s:property value="questionnaireProject.organization"/>"  class="form-control" id="organization" placeholder="填写活动举办方" check-type="required" required-message="活动举办方不能为空" />
			            </article>
			            
			             <article class="form-group">
			               <label for="contact">联系方式</label>
			               <input name="contact" value="<s:property value="questionnaireProject.contact"/>"  class="form-control" id="contact" placeholder="填写活动举办方联系方式" check-type="number" required-message="活动举办方联系方式不能为空" />
			            </article>
			              <article class="form-group pr  input-append date form_datetime">
			               <label for="beginDate">开始时间</label>
			               <input id="beginDate" name="beginDate" readonly   value="<s:date name="questionnaireProject.beginDate" format="yyyy-MM-dd hh:mm" />"  class="form-control" id="beginDate" placeholder="填写活动正式开始时间"  check-type="required" required-message="开始时间不能为空"/>
			           ...
			           <article class="form-group pr input-append date form_datetime">
			               ...
			               <input id="endDate" name="endDate" readonly  value="<s:date name="questionnaireProject.endDate" format="yyyy-MM-dd hh:mm" />"  class="form-control" id="endDate" placeholder="填写活动正式结束时间" check-type="required" required-message="结束时间不能为空"/>
			               <span class="add-on"><i class="fa fa-calendar-o" style="color:#ccc;"></i></span>
			            </article>
			              <script type="text/javascript">
		                ...
		                <article class="form-group">
			               ...
			               <textarea name="explain"  class="form-control" id="explain" placeholder="填写活动介绍"  ><s:property value="questionnaireProject.explains"/></textarea>
			            </article>
			            <article class="form-group">
			               <label for="attention">注意事项</label>
			               <textarea name="attention"  class="form-control" id="attention" placeholder="填写活动注意事项"  ><s:property value="questionnaireProject.attention"/></textarea>
			            </article>
			     </form>
			     		<article class="form-group">
			               <s:if test="questionnaireProject !=null">
					          ...
					          info[2]=$("#organization").val();
					           info[3]=$("#contact").val();
					            info[4]=$("#beginDate").val();
					             ...
					             info[8]=1; 
					             var id=$("#id").val();
					             questionnaireManage.updateProject(info,id,function(result){
					               if(result[9]=="1"){
					                 message("提交成功,页面将跳转");
					                 go_to(2,"questionnaireManage!getMyProject");
					               }else{
